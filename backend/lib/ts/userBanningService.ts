...
export class UserBanningService {
  ...
  async getBanStatus(
    ...
    this.setBanStatusToCache(
      userId,
      result.metadata[USER_METADATA_BAN_STATUS_KEY]
    );

    return {
      status: "OK",
      banned: result.metadata[USER_METADATA_BAN_STATUS_KEY],
    };
  }
  ...
  async setBanStatus(
    ...
    await UserMetadata.updateUserMetadata(
      userId,
      { [USER_METADATA_BAN_STATUS_KEY]: !!isBanned },
      userContext
    );
